...
const updateTranscript = async (annotation, id, callback) => {
  ...
  if (callback) {
    ...
    return callback(null, { message: 'Transcript updated successfully', changes: data?.length || 0 });
  }
  return { data, error };
};
...
const validatePasscode = (code, userId) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!userId || !code) return reject(new Error('userId and code are required'));
      ...
      if (error) return reject(new Error(`Database error during passcode validation: ${error.message}`));
      if (!data || data.length === 0) return reject(new Error('Passcode not found or already used'));
      ...
      if (updErr) return reject(new Error(`Failed to mark passcode as used: ${updErr.message}`));
      resolve(true);
    } catch (e) {
      reject(e);
    }
...
const runQuery = async (_sql, _params = []) => {
  throw new Error('runQuery is not supported with Supabase REST. Use a direct SQL client or RPC.');
};

const getAllQuery = async (_sql, _params = []) => {
  throw new Error('getAllQuery is not supported with Supabase REST. Use a direct SQL client or RPC.');
};
